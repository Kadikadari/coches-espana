  ...
  if (!car) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4 text-center">
        <h1 className="text-2xl font-black text-gray-800 mb-4">Lo sentimos, no encontramos el coche.</h1>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="relative bg-black rounded-[2.5rem] h-[350px] md:h-[600px] overflow-hidden group shadow-2xl">
              {car.images && car.images.length > 0 ? (
                    ...
                    <>
                      ...
                      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-2">
                        {car.images.map((_: any, idx: number) => (
                          <div key={idx} className={`h-1.5 rounded-full transition-all duration-300 ${idx === currentImageIndex ? 'w-8 bg-white' : 'w-1.5 bg-white/40'}`} />
                        ))}
                      </div>
                    </>
                  )}
                ...
                <div className="w-full h-full flex items-center justify-center text-gray-500 font-bold">Sin imágenes disponibles</div>
              )}
            </div>
            ...
            <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-sm border border-gray-100">
              ...
              <div className="flex flex-wrap gap-4 mb-10 pb-10 border-b border-gray-100">
                ...
                <div className="bg-orange-50 text-orange-600 px-6 py-3 rounded-2xl font-black flex items-center gap-2">
                ...
                <div className="bg-gray-50 text-gray-600 px-6 py-3 rounded-2xl font-black flex items-center gap-2">
              ...
              <p className="text-gray-600 leading-relaxed text-lg mb-10 whitespace-pre-line font-medium">
                {car.description}
              </p>

              <h2 className="text-2xl font-black mb-6 text-gray-900">Ficha técnica</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-gray-50 p-6 rounded-3xl border border-gray-50">
                  <p className="text-[10px] text-gray-400 uppercase font-black tracking-widest mb-2">Combustible</p>
                  <p className="font-bold text-gray-800 text-xl">{car.fuel || 'N/A'}</p>
                </div>
                <div className="bg-gray-50 p-6 rounded-3xl border border-gray-50">
                  <p className="text-[10px] text-gray-400 uppercase font-black tracking-widest mb-2">Cambio</p>
                  <p className="font-bold text-gray-800 text-xl">{car.transmission || 'N/A'}</p>
                </div>
                <div className="bg-gray-50 p-6 rounded-3xl border border-gray-50">
                  <p className="text-[10px] text-gray-400 uppercase font-black tracking-widest mb-2">Vendedor</p>
                  <p className="font-bold text-gray-800 text-xl truncate">{car.seller || 'Particular'}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="bg-white p-8 md:p-10 rounded-[2.5rem] shadow-2xl shadow-blue-900/5 border border-gray-100 sticky top-24">
              <div className="mb-10">
                <p className="text-gray-400 font-bold mb-2 uppercase text-[10px] tracking-widest">Precio al contado</p>
                <div className="flex items-baseline gap-1">
                  <p className="text-6xl font-black text-blue-600 tracking-tighter">{car.price}</p>
                  <p className="text-2xl font-black text-blue-600">€</p>
                </div>
              </div>

              <div className="space-y-4">
                <a href={`tel:${car.phone}`} className="w-full bg-blue-600 text-white py-5 rounded-[1.5rem] font-black text-lg flex items-center justify-center gap-3 hover:bg-blue-700 transition shadow-xl shadow-blue-200 transform hover:scale-[1.02] active:scale-[0.98]">
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                  LLAMAR AHORA
                </a>

                <a
                  href={`https://wa.me/${car.phone.replace(/\s+/g, '')}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full bg-[#25D366] text-white py-5 rounded-[1.5rem] font-black text-lg flex items-center justify-center gap-3 hover:bg-[#20ba59] transition shadow-xl shadow-green-100 transform hover:scale-[1.02] active:scale-[0.98]"
                >
              ...
              <div className="mt-10 pt-8 border-t border-gray-100 text-center">
                <p className="text-[10px] text-gray-400 uppercase tracking-[0.2em] font-black mb-2">Ubicación</p>
                <p className="text-gray-900 font-bold">{car.location}, España</p>
              </div>
            </div>
          </div>
